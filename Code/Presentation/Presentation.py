...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 26 19:05:01 2018
...
#%%
"""
For the presentation on 29.11.2018
"""
...
H=25.0
dtEu=0.1
dtLa=100  #dt for particle scheme
T=2*3600 #one hours
...
E1Start=np.random.normal(mu, sigma, Np)
ConcStart=Gaussian(zEu,mu,sigma)

fig, ax = plt.subplots(1)
n, bins, patches = ax.hist(E1Start, Num_O_Bins_Poi, density=1, 
                           label="Lagrangian, Euler-Maruyama SDE $t=0$ seconds", alpha=0.7)
ax.plot(zEu, ConcStart, '--', label="Eulerian, Duffusion PDE $t=0$ seconds", alpha=0.7)

n, bins, patches = ax.hist(E1, Num_O_Bins_Poi, density=1,
                           label="Lagrangian Euler-Maruyama SDE $t=2*3600$ seconds",alpha=0.7)
ax.plot(zEu,Conc, '--', label="Eulerian, Duffusion PDE $t=2*3600$ seconds", alpha=0.7)

ax.set_xlabel('Depth [m]')
ax.set_ylabel('Consentration [1/m]')
ax.set_title(r'PDE v.s SDE')
ax.legend()
plt.tight_layout()
